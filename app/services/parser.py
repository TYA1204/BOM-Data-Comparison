...
_META_KEYWORDS = ['用户：', '有效日期从', '日期：', '时间：', 'BOM状态', 'BOM工厂', '页码：']
_COL_HEADER_KEYWORDS = ['组  件', '组件', '组   件']
_PN_PATTERN = re.compile(r'^[A-Z]\d{6}-\d{6}-\d{2,3}\w*$')


def _is_sap_bom_tab(text):
    """Check if raw text is SAP BOM 展开表 (UTF-16 LE tab-separated)."""
    return 'B O M 展开表' in text or 'BOM展开表' in text


def _parse_sap_bom_lines(text):
    """Parse SAP BOM 展开表 raw text into structured data rows.

    Returns (metadata_dict, data_rows_list).
    Each data_row is a dict with keys: level, part_number, part_name,
    quantity, unit, priority, ecn, notes, is_alternative.
    """
    lines = text.split('\r\n')
    metadata = {}
    data_rows = []
    ...
    for line in lines:
        ...
        # --- Extract metadata ---
        if '顶层BOM号' in stripped:
            ne = [p.strip() for p in parts if p.strip()]
            if len(ne) >= 2:
                metadata['bom_number'] = ne[1]
            continue
        if '最后更改号' in stripped:
            ne = [p.strip() for p in parts if p.strip()]
            if len(ne) >= 2:
                metadata['ecn'] = ne[1]
            continue
        if 'BOM工厂' in stripped:
            ne = [p.strip() for p in parts if p.strip()]
            if len(ne) >= 2:
                metadata['factory'] = ne[1]
            continue
        if '有效日期从' in stripped:
            ne = [p.strip() for p in parts if p.strip()]
            if len(ne) >= 2:
                metadata['valid_from'] = ne[1]
            continue
        # Skip other metadata / column headers / page headers
        if any(kw in stripped for kw in _META_KEYWORDS + _COL_HEADER_KEYWORDS):
            continue

        # --- Sub-assembly header row (parent item, no level number) ---
        # Format: part_number\t\t...\tdescription  (11 fields)
        if len(parts) <= 15 and _PN_PATTERN.match(stripped.split('\t')[0].strip()):
            pn = parts[0].strip()
            desc = ''
            for p in parts[1:]:
                p = p.strip()
                if p and not _PN_PATTERN.match(p):
                    desc = p
                    break
            # Record as a "parent marker" row for hierarchy context
            data_rows.append({
                'level': 0, 'part_number': pn, 'part_name': desc,
                'quantity': '', 'unit': '', 'priority': '', 'ecn': '',
                'notes': '', 'is_alternative': False, '_type': 'header'
            })
            continue

        # --- Data row: starts with level number ---
        if not re.match(r'^\d+\t', stripped):
            continue

        non_empty = [(j, p.strip()) for j, p in enumerate(parts) if p.strip()]
        if len(non_empty) < 3:
            continue

        level_str = non_empty[0][1]
        ...
        if len(non_empty) > 1 and non_empty[1][1] == '#':
            is_alt = True
            pn = non_empty[2][1]
        else:
            pn = non_empty[1][1]
...
def parse_sap_bom_file(file_path):
    """Parse SAP BOM 展开表 (.xls disguise) and return DataFrame.

    Handles UTF-16 LE tab-separated SAP export with multi-page headers.
    """
    with open(file_path, 'rb') as f:
        raw = f.read()

    # Try UTF-16 LE first (most common SAP export)
    try:
    ...
    metadata, data_rows = _parse_sap_bom_lines(text)

    # Filter to only data rows (skip sub-assembly headers)
    items = [r for r in data_rows if r['_type'] == 'data']
    if not items:
    ...
    # Add bom_name from metadata
    bom_name = metadata.get('bom_number', '')
    bom_version = metadata.get('ecn', '')

    return df, {'bom_name': bom_name, 'bom_version': bom_version, 'metadata': metadata}
