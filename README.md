...
├── docs/                    # 技术文档
│   └── WORD导出逻辑文档.md    # WORD 导出保姆级逻辑文档
├── SOP_BOM核对操作指南.md     # 用户操作 SOP
├── wsgi.py                  # Gunicorn 入口
...
- `unit` — 单位变更
- `version` — 版本变更（仅版本比对）
- `reference` — 位号变更（同 PN + 同父件，PCB 位置增删，v2026-06-27 新增）

### 4. 差异报告导出

#### WORD 更改通知单
- 按叶子直接父组件归组，H5F 叶子按功能键合并到 P3EM 同功能父组件下
- 中间组件（PN 同时是父件）自动跳过，只留叶子差异
- 物料名称自动清洗（去品牌/认证/工艺/紧固件描述等噪声）
- 父组件头行蓝色粗体，物料行深灰色，用量制表位对齐
- **位号变更**：直接归类到组件 ADD/DEL 区（新增→ADD 行，删除→DEL 行）；同 PN 同时有数量和位号变更时，位号优先，数量行自动隐藏
- 表头字段：机芯/机型/日期/订单号/阶段/数量，全自动填充
- 说明栏动态话术：「此份差异核对结果来源于 XX（源BOM）与 YY（目标BOM）的 ZZ 阶段比对，仅适用于 YY（目标机型）使用。」
- 拟制/审核人：导出前在侧边栏卡片填写，localStorage 记忆
- 文件名：`{订单号}_{机芯}_{机型}_{数量}台_{阶段}_整机更改通知单.docx`

#### Excel 报告
- 结构化差异列表导出，含机芯/机型/日期/阶段信息行
...
选中组件 → 全层级递归收集子树 (_collect_subtree)
         → skip_pns 前置递归排除（不勾的组件+子孙完全移除）
         → 计算 parent_pns（基于全量 BOM 数据）
         → [可选] exclude_leaves: 排除不在 parent_pns 中的项
         → 逐项比对:
           Step 1: 新增（PN in B not in A）
           Step 2: 删除（PN in A not in B）
           Step 3: 变更（同 PN + 同父件 → 用量/单位/位号/版本变化）
         → [可选] skip_pns 后置过滤（结果中移除命中 PN）
         → 输出差异记录
```

## WORD 导出逻辑

详见 [`docs/WORD导出逻辑文档.md`](docs/WORD导出逻辑文档.md)，完整覆盖：
- 前端导出配置卡片 + 必填校验
- 四层后端处理（取数 → 洗名 → 归组 → 填表）
- 物料名称四道清洗（精确噪声 / 模式匹配 / 复合拆解 / 截断标记）
- 父组件名截短 + 用量制表位对齐
