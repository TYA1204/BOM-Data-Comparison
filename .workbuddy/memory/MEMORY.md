...
- **任务完成前必须验证**：代码改完部署后，在服务器上跑验证脚本或模拟测试确认结果正确，**不把验证工作交给用户**，确认通过才说"完成"。
- **指令即边界，不得自行扩展**：用户说删 A 就只删 A，不能因为 B 在同一个区域就顺手删掉。用户指令是白名单，不是启发式建议。
- **批量删除前必须确认清单**：涉及删除文件/路由/功能时，先列出完整删除清单，等用户确认再动手。不替用户做"这个也应该删"的判断。
